...
public class Player extends GameObject {
	private static final Clips[] STEP_SOUNDS = new Clips[] { Sound.step1,
			Sound.step2, Sound.step3, Sound.step4, Sound.step5, Sound.step6,
			Sound.step7, };
	private static final Clips[] JUMP_SOUNDS = new Clips[] { Sound.jump1,
			Sound.jump2, Sound.jump3, Sound.jump4, };
	private static final Clips[] LAND_SOUNDS = new Clips[] { Sound.land1,
	...
	public void update() {
		...
		if (alive) {
			...
			if (standing) {
				if (!isRunning) {
					setSpriteAction("default", new String[] { "land",
							"conjure", "default" });
				}
			} else {
	...
	private void toggleSwitch() {
		Switch sw = getGame().findObject(Switch.class, position.x - 0.5,
				position.y - 0.5, 1, 1, true);
		if (sw != null) {
	...
	private boolean isAreaClear(Vector2 pos) {
		...
		if (pos.x < 0 || pos.y < 0 || pos.x >= map.getWidth()
				|| pos.y >= map.getHeight()) {
			return false;
		}
		...
		return !map.getTile(pos).getCollidable()
	...
	public void jump() {
		...
		if (jumpsLeft > 0) {
			...
			getGame().playSoundAtLocation(Sound.airjump, position,
					standing ? 0.4 : 0.6);
		}
	}
