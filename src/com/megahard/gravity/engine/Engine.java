...
public class Engine implements KeyListener, MouseListener, MouseMotionListener,
	 ...
	 * 
	 * @see
	 * com.megahard.gravity.GameContext#addObject(com.megahard.gravity.GameObject
	 * )
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.megahard.gravity.GameContext#removeObject(com.megahard.gravity.GameObject
	 * )
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.megahard.gravity.GameContext#playSoundAtLocation(com.megahard.gravity
	 * .Sound.Clips, double, double, double)
	 */
	@Override
	public void playSoundAtLocation(Clips sound, double x, double y,
			double volume) {
	 ...
	 * @see
	 * com.megahard.gravity.GameContext#playSoundAtLocation(com.megahard.gravity
	 * .Sound.Clips, com.megahard.gravity.Vector2, double)
	 */
	@Override
	public void playSoundAtLocation(Clips sound, Vector2 position, double volume) {
		...
		float p = (float) (1 - Math.atan2(3, position.x
	...
	private void loadScript(GameObjects object) {
		...
		try {
			Script s = null;
			Rectangle2D.Double region = new Rectangle2D.Double(object.getX()
					/ Renderer.TILE_SIZE, object.getY() / Renderer.TILE_SIZE,
					object.getWidth() / Renderer.TILE_SIZE, object.getHeight()
							/ Renderer.TILE_SIZE);
			...
			try {
				constructor = subclass.getConstructor(GameContext.class,
						Rectangle2D.Double.class, Map.class);
	...
	private void updateScripts() {
		for (Script s : state.scripts) {
			...
			List<GameObject> objects = findObjects(s.getRegion().x,
					s.getRegion().y, s.getRegion().width, s.getRegion().height,
					true);
	...
	private void updateInputEvents() {
		while (true) {
			...
			if (ke.state == KeyState.PRESS) {
				...
				keyStates.put(keyCode, prev == KeyState.PRESS ? KeyState.CLICK
						: KeyState.RELEASE);
		}
		...
		while (true) {
			...
			if (ke.state == KeyState.PRESS) {
				...
				mouseStates.put(button, prev == KeyState.PRESS ? KeyState.CLICK
						: KeyState.RELEASE);
		}
	}
	...
	public boolean keyIsUp(int keyCode) {
		KeyState state = keyStates.get(keyCode);
		return state == null || state == KeyState.RELEASE
				|| state == KeyState.UP;
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.megahard.gravity.GameContext#findObject(java.lang.Class, double,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.megahard.gravity.GameContext#findObjects(double, double, double,
	 * double, boolean)
	 */
	@Override
